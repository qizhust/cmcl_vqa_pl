...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.detr_transform = False
        for transform_key in transform_keys:
            if 'clip' in transform_key:
                self.clip_transform = True
                break
        for transform_key in transform_keys:
            if 'detr' in transform_key:
                self.detr_transform = True
                break
    ...
    def get_image(self, index, image_key="image"):
        ...
        if self.detr_transform:  # pixel_values: [(3, h1, w1), (3, h2, w2)]
            image_tensor = [tr(image).pixel_values[0] for tr in self.transforms]
        else:
    ...
    def get_false_image(self, rep, image_key="image"):
        ...
        if self.detr_transform:
            image_tensor = [tr(image).pixel_values[0] for tr in self.transforms]
        else:
            image_tensor = [tr(image) for tr in self.transforms]
        return {f"false_image_{rep}": image_tensor, 'raw_false_Image': image}
    ...
    def collate(self, batch, mlm_collator, pos_model):
        ...
        for img_key in img_keys:  # ['false_image_0', 'image']
        ...
        if len(txt_keys) != 0:
            ...
            # flatten_texts = [e for text in texts for e in text]
