...
def compute_mlm_oracle(pl_module, batch):
    ...
    loss_mask = (mlm_labels>-1).float()

    mlm_oracle_logits = pl_module.infer_oracle_logits(batch, mask_text=True)
    mlm_oracle_loss = F.kl_div(mlm_logits.softmax(-1).log(), mlm_oracle_logits.softmax(-1), reduction='none')
    mlm_oracle_loss = (mlm_oracle_loss.sum(-1) * loss_mask).sum()/(loss_mask.sum() + 1e-5)

    ret = {
        "mlm_oracle_loss": mlm_oracle_loss,
        "mlm_oracle_logits": mlm_oracle_logits,
        "mlm_logits": mlm_logits,
        "mlm_labels": mlm_labels,
    }

    phase = "train" if pl_module.training else "val"
    loss = getattr(pl_module, f"{phase}_mlm_oracle_loss")(ret["mlm_oracle_loss"])
    acc = getattr(pl_module, f"{phase}_mlm_oracle_accuracy")(
        ret["mlm_oracle_logits"], ret["mlm_labels"]
    )
    pl_module.log(f"mlm_oracle/{phase}/loss", loss)
    pl_module.log(f"mlm_oracle/{phase}/accuracy", acc)
...
def compute_mlm(pl_module, batch):
    ...
    if pl_module.hparams.config["focal"]:
        tmp_prob = mlm_logits.softmax(-1).view(-1, pl_module.hparams.config["vocab_size"])
        tmp_labels = mlm_labels.view(-1).clone().unsqueeze(-1)
        tmp_labels[tmp_labels < 0] = 0
        mlm_loss = (1 - torch.gather(tmp_prob, 1, tmp_labels).squeeze(-1)
                )**pl_module.hparams.config["focal_gamma"] * mlm_loss
...
def compute_vqa(pl_module, batch):
    ...
    vqa_loss = vqa_loss_sup + pl_module.hparams.config["nce_lambda"]*infer['loss_infonce']

    ret = {
        "vqa_loss": vqa_loss,
        "vqa_l_sup": vqa_loss_sup,
        "vqa_l_infonce": infer['loss_infonce'],
    ...
    loss_sup = getattr(pl_module, f"{phase}_vqa_l_sup")(ret["vqa_l_sup"])
    loss_infonce = getattr(pl_module, f"{phase}_vqa_l_cl")(ret["vqa_l_infonce"])
    score = getattr(pl_module, f"{phase}_vqa_score")(
        ret["vqa_logits"], ret["vqa_targets"]
    )
    pl_module.log(f"vqa/{phase}/loss", loss)
    pl_module.log(f"vqa/{phase}/loss_sup", loss_sup)
    pl_module.log(f"vqa/{phase}/loss_cl", loss_infonce)
...
def vqa_test_wrapup(outs, model_name, save_path='/workspace/src/METER-main'):
    ...
    with open(f"{save_path}/vqa_submit_{rank}.json", "w") as fp:
    ...
    if rank == 0:
        jsons = list()
        paths = list(glob.glob(save_path+"/vqa_submit_*.json"))
        for path in paths:
            with open(path, "r") as fp:
                jsons += json.load(fp)
        os.makedirs(os.path.join(save_path, "result"), exist_ok=True)
        with open(f"{save_path}/result/vqa_submit_{model_name}.json", "w") as fp:
            json.dump(jsons, fp, indent=4)

    torch.distributed.barrier()
    os.remove(f"{save_path}/vqa_submit_{rank}.json")


def arc_test_wrapup(outs, caplen, model_name, save_path='/workspace/src/METER-main'):
    rank = torch.distributed.get_rank()
    ...
    with open(f"{save_path}/coco_cap_len{caplen}_{rank}.json", "w") as fp:
        json.dump(rets, fp, indent=4)
    ...
    if rank == 0:
        jsons = list()
        paths = list(glob.glob(f"{save_path}/coco_cap_len{caplen}_*.json"))
        for path in paths:
            with open(path, "r") as fp:
                jsons += json.load(fp)
        os.makedirs(os.path.join(save_path, "result/arc"), exist_ok=True)
        jsons = sorted(jsons, key=lambda x: x["image_id"])
        with open(f"{save_path}/result/arc/coco_cap_{model_name}_len{caplen}.json", "w") as fp:
            json.dump(jsons, fp, indent=4)

    torch.distributed.barrier()
    os.remove(f"{save_path}/coco_cap_len{caplen}_{rank}.json")
