    ...
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                setattr(pl_module, f"{split}_{k}_l_sup", Scalar())
                setattr(pl_module, f"{split}_{k}_l_cl", Scalar())
            elif k == "nlvr2":
                if split == "train":
...
def epoch_wrapup(pl_module):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        if v < 1:  # as regularization terms, not contribute to final acc metric
            continue
...
def set_task(pl_module):
    ...
    pl_module.current_tasks_w = dict(
        [(k+"_loss", v) for k, v in pl_module.hparams.config["loss_names"].items() if v > 0]
    )
    return
